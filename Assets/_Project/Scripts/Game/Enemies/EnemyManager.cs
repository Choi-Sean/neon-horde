    ...
    public sealed class EnemyManager : MonoBehaviour
    {
        ...
        void LateUpdate()
        {
            ...
            for (int i = 0; i < _count; i++)
            {
                ...
                _pool.Draw(_glowSprite, new Vector2(e.pos.x, e.pos.y - bob * 0.3f), 0f,
                    col * (boss ? 0.9f : 0.5f), sc * (boss ? 1.9f : 1.5f));

                _pool.Draw(fr, new Vector2(e.pos.x, e.pos.y + bob), -0.1f, col,
                    new Vector2(faceX * sc, sc));
            }
            _pool.End();
        }
    }
